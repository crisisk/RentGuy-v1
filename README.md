...
- React Router v6 configuration with lazy-loaded routes and guard metadata lives in `src/router/routes.tsx`, ensuring every console view has a defined path and fallback.【F:src/router/routes.tsx†L1-L176】
- The planner shell consumes Zustand-driven state for responsive menus and onboarding flows, as implemented in `src/ui/Planner.tsx` with helpers from the shared stores.【F:src/ui/Planner.tsx†L21-L45】【F:src/ui/Planner.tsx†L561-L624】
- Global authentication state, persistence, and guard hooks are implemented in `src/stores/authStore.ts`, aligning with the documented state-management scope.【F:src/stores/authStore.ts†L1-L186】
...
- Business-logic orchestration for project workflows now lives alongside the pages, e.g. `src/pages/project/ProjectOverview.tsx` and `src/pages/project/ProjectDetails.tsx` coordinate filtering, formatting, and data hydration against the shared stores.【F:src/pages/project/ProjectOverview.tsx†L1-L124】【F:src/pages/project/ProjectDetails.tsx†L1-L80】
...
| P1       | Repair TypeScript compiler errors across UI, stores, and tests | Complete    | `npm run typecheck` now completes cleanly after tightening analytics helpers, CRM/project views, the finance store, and onboarding planner types against the shared contracts.【F:src/utils/analytics.ts†L1-L171】【F:src/pages/crm/CustomerForm.tsx†L1-L200】【F:src/stores/financeStore.ts†L1-L460】【F:src/ui/OnboardingOverlay.tsx†L420-L1700】【6a687c†L1-L5】 |
