...
- **Auth client**: `src/application/auth/api.ts` centralises login, session refresh, and role updates with `AppError`/`Result` wrappers so UI flows consume structured success/error contracts.
- **Barcode Scanner & Marketing Modes**: Alternative entries in `src/ui/Scanner.tsx` and `src/ui/MarketingLandingPage.tsx` toggled via `VITE_APP_MODE` (or a runtime `?mode=` override), now typed end-to-end and backed by the structured offline queue helper (`@infra/offline-queue`) so AppError metadata flows door de UI.
...
| --- | --- | --- |
| Type Safety | TS strict config exists, maar kritieke views bevatten nog JavaScript (`src/ui/Planner.jsx`). Backend lacks mypy. | No typed DTOs, no shared contracts, unchecked axios usage beyond the typed shell, missing env typing for frontend feature modules. |
...
| --- | --- | --- | --- | --- | --- |
| `src/main.tsx`, `src/ui/App.tsx`, `src/ui/Login.tsx`, `src/ui/OnboardingOverlay.tsx`, `src/ui/Planner.jsx`, `src/application/auth/api.ts` | React shell components typed; onboarding overlay en login flow consumeren nu `Result<AppError>`-contracten maar planner-flow blijft implicit en deelt mutable state | Breid codemod uit naar planner, extraheer typed hooks voor storage access en deel API DTO contracts. Calendar toggle houdt legacy drag/drop gedrag actief; forwardRef-proxies (`Planner.jsx`, `OnboardingOverlay.jsx`) blijven bestaan voor langlopende branches en vereisen rooktests. | Type Safety, Maintainability, Error Handling, Test Coverage | Medium | Medium |
| `src/ui/Scanner.tsx`, `warehouse/offline-queue.js` | Runtime now typed and resilient but lacks automated coverage and telemetry exports | Add Vitest suites for queue persistence/flush semantics, emit structured analytics events from scanner interactions, document offline sync runbook | Error Handling, Test Coverage, Documentation | Medium | Medium |
