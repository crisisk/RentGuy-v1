...
| --- | --- | --- | --- | --- | --- |
| `src/main.tsx`, `src/ui/App.tsx`, `src/ui/Login.tsx`, `src/ui/Planner.jsx` | React shell components typed; onboarding overlay geporteerd naar TypeScript maar planner-flow blijft implicit en deelt mutable state | Breid codemod uit naar planner, extraheer typed hooks voor storage access en deel API DTO contracts. Calendar toggle houdt legacy drag/drop gedrag actief; forwardRef-proxies (`Planner.jsx`, `OnboardingOverlay.jsx`) blijven bestaan voor langlopende branches en vereisen rooktests. | Type Safety, Maintainability, Test Coverage | Medium | Medium |
