...
- **Barcode Scanner Mode**: Alternative entry in `src/ui/Scanner.tsx` toggled via `VITE_APP_MODE`, now typed end-to-end and backed by the structured offline queue helper (`@infra/offline-queue`) so AppError metadata flows through the UI.
