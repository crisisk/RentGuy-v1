...
const FLOW_DEFINITIONS: Record<FlowNavigationKey, Omit<FlowExperienceNavRailItem, 'id' | 'status'>> = {
  login: {
    label: 'Inloggen',
    description: 'Activeer je pilottoegang of gebruik een demo-account.',
    href: '/login',
    icon: '🔐',
    badge: 'Stap 1',
  },
  role: {
    ...
    description: 'Kies de persona zodat explainers en dashboards klaarstaan.',
...
export function describeFlowStatus(status: FlowNavigationStatus): string {
  switch (status) {
    case 'complete':
      return 'Flow is afgerond'
    case 'current':
      return 'Flow is actief'
    case 'blocked':
      return 'Flow is geblokkeerd tot de voorwaarden zijn vervuld'
    default:
      return 'Flow staat gepland als volgende stap'
  }
}
