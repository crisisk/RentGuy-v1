...
const ProjectTimeline: React.FC = () => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      <h2 className="text-xl font-semibold mb-6 text-gray-800">Project Timeline</h2>
      ...
      {events.length === 0 ? (
        ...
        <div className="space-y-4">
          {events.map((event, index) => (
            <div key={event.id} className="flex items-start space-x-4 group">
              <div className="flex flex-col items-center">
                <div className="w-2 h-2 bg-blue-500 rounded-full" />
                {index !== events.length - 1 && <div className="w-px bg-gray-200 flex-1 my-2" />}
              </div>
